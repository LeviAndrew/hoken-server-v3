...
export class GameBaseController extends Source {
  ...
  private async create(msg) {
    ...
    try {
      ...
      game.groups.forEach(group => {
        gameToSave.teams.push({nick: group.name, players: []});
      });
      const savedGame = await this.sendToServer('db.gameBase.create', gameToSave);
      if (!savedGame.data.success || savedGame.data.error) return this.answer(msg.id, msg.event, null, 'createGameError');
      this.gameMap.set(game.userId, new Game(savedGame.data.success[0].gameSetting, savedGame.data.success[0].teams, savedGame.data.success[0].id, savedGame.data.success[0].pin));
  ...
  private playerReconnect(msg) {
    if (msg.source_id === this.id) return;
    const event = 'player.reconnectBase.error';
    try {
  ...
  private async checkExistPlayer(playerData: playerData) {
    try {
      const
        gameDB = await this.sendToServer('db.gameBase.read', new FindObject({
  ...
  private async readStartedGames() {
    const
      ret = await this.sendToServer('db.gameBase.read', new FindObject({
        query: {
          $or: [
            {
              gameStatus: 'created',
    ...
    const
      gamesId = ret.data.success.map(game => game.id);
    await this.sendToServer('db.gameBase.update', new UpdateObject({
      query: {
        _id: {
          $in: gamesId,
        }
  ...
  private createGameRoutine(game) {
    this.gameMap.set(game.teacher.toString(), new Game(game.gameSetting, game.teams, game.id, game.pin));
  }
