...
export class GameController extends BasicHandler {
  ...
  private async create(msg) {
    ...
    try {
      ...
      const savedGame = await this.sendToServer('db.game.create', gameToSave);
  ...
  private async readStartedGames() {
    ...
    await this.sendToServer('db.game.update', new UpdateObject({
      query: {
        _id: {
          $in: gamesId,
        }
