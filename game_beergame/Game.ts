...
export class Game extends BasicHandler {
  ...
  private async onStudentDisconnect(teamId, playerId) {
    ...
    this.teacherSocket.emit('student.disconnect', {
      teamId,
      playerId,
    });
  }
  ...
  private async changeStatus(msg) {
    const
      responseEvent = 'game.status.changed';
    try {
  ...
  private async removeGroup(msg) {
    ...
    try {
      ...
      const
        ret = await this.sendToServer('db.game.removeTeam', {
          teamId: msg.teamId,
          gameId: this.gameId
        });
  ...
  private async removePlayer(msg) {
    ...
    try {
      ...
      const
        ret = await this.sendToServer('db.game.removePlayer', {
          teamId: msg.teamId,
          gameId: this.gameId,
  ...
  private async updateStatus(status) {
    ...
    const
      gameDB = await this.sendToServer('db.game.read', new FindObject({
        query: this.gameId,
        select: 'gameStatus',
      }));
  ...
  private async teacherDisconnect(msg) {
    try {
      ...
      const
        response = {success: ret.data.success};
      this.sendToGroups("teacher.disconnect", response);
    } catch (e) {
      return e;
    }
  }
  ...
  private async onTeamFinishedPlayed(team) {
    try {
      ...
      const
        ret = await this.sendToServer('db.game.update', new UpdateObject({
          query: this.gameId,
          update: {
            gameStatus: 'finished',
          }
        }));
      if (!ret.data.success || ret.data.error) return this.teacherSocket.emit('game.finished', {error: 'weCannotFinishedGame'});
      this.teacherSocket.emit('game.finished', {success: true,});
    } catch (e) {
      console.error(e);
    }
  }
  ...
  private async asyncGame(team) {
    ...
    const
      updateGame = await this.sendToServer('db.game.update', new UpdateObject({
        query: {
          "_id": this.gameId,
    ...
    const
      updatedGame = await this.sendToServer('db.game.read', new FindObject({
        query: this.gameId,
        select: 'teams'
      }));
  ...
  private teacherWiring() {
    this.teacherSocket.on("game.group.remove", this.removeGroup.bind(this));
    this.teacherSocket.on("game.player.remove", this.removePlayer.bind(this));
    this.teacherSocket.on("game.status.change", this.changeStatus.bind(this));
    this.teacherSocket.on("disconnect", this.teacherDisconnect.bind(this));
  }
