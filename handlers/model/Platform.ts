...
export class Platform extends BasicHandler {
    ...
    public async createGameLogis(param: defaultParam<{ teams: any[] }>) {
      const model = 'user',
        required = this.attributeValidator([
          "pKey", "userId", "data",
          [
      ...
      try {
        ...
        const
          settingPromises = await Promise.all([
            this.sendToServer('db.user.read', new FindObject({
              query: param.userId,
        ...
        for (let i = 0; i < promises.length; i++) {
          if (!promises[i].data.success || promises[i].data.error) return await this.returnHandler({
            model,
            data: {error: 'createGameError'}
          });
        }
        const
          gameConfig = settingPromises[0].data.success.userSettings ? settingPromises[0].data.success.userSettings : settingPromises[1].data.success;
        const
          initPlayed = this.getPlayedStart(gameConfig),
          groups = [];
        for (let i = 0; i < param.data.teams.length; i++) {
          let
            group = {
        ...
        const
          ret = await this.basicCreateGameLogis(param.userId, groups, true);
        if (!ret.data.success || ret.data.error) return await this.returnHandler({
          model,
          data: {error: 'cannotCreateGame'}
        });
    ...
    public async getAvailableReportsLogis(param: defaultParam<null>) {
      const model = 'user',
        required = this.attributeValidator([
          "pKey", "userId",
        ], param);
    ...
    public async readGameDetailLogis(param: defaultParam<{ id: string }>) {
      const model = 'user',
        required = this.attributeValidator([
          "pKey", "userId", 'data',
          [
    ...
    public async jsonToXLSXLogis(param: defaultParam<{ id: string }>) {
      const model = 'user',
        required = this.attributeValidator([
          "pKey", "userId", 'data',
          [
