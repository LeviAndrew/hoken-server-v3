...
describe('10.2.GameBase_Game', () => {
  ...
  describe('CREATE', () => {
    ...
    describe('TEST', () => {
        ...
        chai.request(baseURL)
          ...
          .end((error, response) => {
            ...
            response.body.data.forEach(game => {
              ...
              expect(game).to.have.all.keys("removed", "gameSetting", "teacher", "teams", "id", "createdAt", "updatedAt", "gameStatus", "pin");
            });
      ...
      it('try create another game', (done) => {
        chai.request(baseURL)
          .post("/api/user/gameBase")
          .set('authentication-key', loggedUser.authenticationKey)
          .set('access-key', loggedUser.accessKey)
  ...
  describe('CONNECT', () => {
    ...
    describe('TEST', () => {
      ...
      it('OK', (done) => {
        ...
        clientIO.on("student.enterBase", studentEnter);
        const
          data = {
  ...
  describe('READ AVAILABLE GAME', () => {
    ...
    describe('TEST', () => {
      ...
      it('OK', (done) => {
        chai.request(baseURL)
          .get("/api/open/available-gameBase")
          .end((error, response) => {
            expect(response.body).to.be.instanceof(Object);
            expect(response.body).to.have.all.keys("success", "data");
            expect(response.body.success).to.be.true;
      ...
      it('OK', (done) => {
        const
          response = (msg) => {
            ...
            expect(msg.response.success).to.be.true;
            expect(msg.response.data).to.be.true;
            playerIO.on("decision.receivedBase", (msg) => {
              done();
            })
            playerIO.emit("player.takeDecisionBase", {request: data});
          };
        ...
        chai.request(baseURL)
          ...
          .end((error, response) => {
            for (let i = 0; i < response.body.data[0].teams.length; i++) {
              if (response.body.data[0].teams[i].players.length < response.body.data[0].gameSetting.playersPerTeam) {
                playerCount++;
                game = {
  ...
  describe('TEACHER START GAME', () => {
    ...
    const onEvent = "gameBase.status.changed";
    ...
    describe('TEST', () => {
      ...
      it('OK', (done) => {
        ...
        clientIO.emit("gameBase.status.change", {request: data});
      });
  ...
  describe('TEACHER DISCONNECT', () => {
    ...
    const onEvent = "teacher.disconnectBase";
  ...
  describe('STUDENT DISCONNECT', () => {
    ...
    const onEvent = "student.disconnectBase";
    ...
    describe('TEST', () => {
      ...
      it('OK', (done) => {
        const
          response = (msg) => {
            expect(msg).to.be.instanceOf(Object);
            expect(msg).to.have.all.keys("playerId", "gameId", "teamId", "nick", "teacher", "playedArray");
            done();
          };
